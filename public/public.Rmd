...
* How can researchers use *all* data available to them -- even if they come from very different studies -- for inference and discovery?
...
\begin{align}
 \log(\mathit{Cl}_{j, s}) &\sim \mathcal{N}(\mu_{\mathit{Cl}} + \mathbf{x}_{j, s}^{\mathit{Cl}} \beta_{\mathit{Cl}}, \sigma^2_{Cl})\\
 \log(t_{\max,j, s}) &\sim \mathcal{N}(\mu_{t} + \mathbf{x}_{j, s}^{t} \beta_{t}, \sigma^2_{t}) \\
 \operatorname{logit}(\alpha_{j, s}) &\sim \mathcal{N}(\mu_{\alpha} + \mathbf{x}_{j, s}^\alpha \beta_{\alpha}, \sigma^2_{\alpha})\\
 \operatorname{logit}(F_{j, s}) &\sim \mathcal{N}(\mu_{F} + \mathbf{x}_{j, s}^{F} \beta_{F}, \sigma^2_F)
\end{align}
