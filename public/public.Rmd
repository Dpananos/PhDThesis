...
2. Framework for "is the juice worth the squeeze".
  * Simulation-based evaluation of personalization based on PK combined with DTRs.
3. Demonstrate how PM researchers in academic centers can use all data available to them.
...
logit_p <- seq(-5, 5, 0.01)
log_post <- -log_posterior(logit_p)
map_approx <- r$value - as.numeric(r$hessian)*(logit_p - r$par)^2
...
tibble(
  ...
  labs(x='p',
       ...
       caption = "Prior for p Beta(10, 90), y = 10, n = 100")
...
$$ \dfrac{dC}{dt} = \dfrac{FDk_a}{V}\exp(-k_a t) - k_e C(t) $$



* Can solve this ODE, substituting $V = Cl/k_e$
...
* Patient comes in and get's their clinical vars measured ($O_1$)
* Dose is selected based on some decision rule ($A_1$)
* Patient (possibly) returns for a blood measurement ($O_2$)
* Dose is (possible) adjusted ($A_2$)
* Value: "Proportion of time spent in some concentration range"
...
* Want to assume *something* is common between the two data, but what?
